...
(function() {
  jQuery(function($) {
    ...
    return $('.comment_edit').click(function() {
      var form, id;
      id = this.id.split('_')[1];
      form = this.form;
      return $.get("/needs/(＾０＾)/comments/" + id + ".json").success(function(response) {
        var k, method, original_action, v;
        ...
        $("#new_comment").attr({
          action: "/needs/(＾０＾)/comments/" + id
        });
        if (!$("#new_comment input[name=_method]").get(0)) {
          method = $('<input>').attr({
            type: 'hidden',
            name: '_method'
          }).val('put');
          return $("#new_comment").prepend(method);
        }
      });
    });
  });
